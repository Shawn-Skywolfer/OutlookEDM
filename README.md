...
- **中英双语界面**：点击页面右上角的 `English` 或 `中文` 按钮切换语言，语言选择会保存在浏览器中。
- **上传切片模式**：上传多张图片、自动排序、拖拽调整顺序、设置邮件显示宽度并生成 EML。
- **长图切割模式**：在长图上创建、调整和删除横向切片，支持吸附、尺寸提示、撤销与重做。
- **Outlook 预览**：按实际邮件宽度预览；内容超宽时可使用横向滚动条。
- **EML 导出**：将图片作为 CID 内嵌资源写入邮件，不依赖远程图片地址。
